...
namespace Mashed_Poogie
{
    public class JobGiver_SniffOutItem : ThinkNode_JobGiver
    {
		...
		{
            if (c.GetTerrain(pawn.Map).affordances.NullOrEmpty() || !c.GetTerrain(pawn.Map).affordances.Contains(TerrainAffordanceDefOf.Diggable))
			{
				return false;
			}
            if (!c.StandableBy(pawn.Map, pawn))
			{
                return false;
			}
